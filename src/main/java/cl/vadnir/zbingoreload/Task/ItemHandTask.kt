...
class ItemHandTask(private val plugin: ZBingoReload) {
    init {
        Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, {
            ...
            for (player in this.plugin.getPlayerManager().getAllPlayers()){
                ...
                val team: TeamProfile = this.plugin.getTeamManager().getTeam(player.getTeamId())?: continue

                val advancement: AdvancementProfile = this.plugin.getAdvancementManager()
                    .getAdvancement(
                        Bukkit.getPlayer(player.getName())!!.inventory.itemInMainHand.type.name,
                        AdvancementTypes.Items) ?: continue
